...
def gen_multi_level_local_graph_v3(
    ...
    for config in level_configs:
        ...
        vertices, vertices_num = gen_graph_fn(points_xyz, center_xyz, **method_kwarg)
        edges_list.append(vertices)
        edges_num_list.append(vertices_num)

    #adj_local, adj_global = convert_sparse_matrix(vertex_coord_list, keypoint_indices_list, edges_list)

    return vertex_coord_list, keypoint_indices_list, edges_list, edges_num_list


def gen_disjointed_rnn_local_graph_gcn(
    ...
    vertices_num = [neighbors.size for i, neighbors in enumerate(indices)]
...
def get_graph_generate_fn(method_name):
    method_map = {
        'disjointed_rnn_local_graph_v3':gen_disjointed_rnn_local_graph_gcn,
