...
def boxes_3d_to_corners(boxes_3d):
    ...
    for box_3d in boxes_3d:
        x3d, y3d, z3d, l, h, w, qw, qx, qy, qz = box_3d   # qw, qx, qy, qz

        qm = qTOm([qw, qx, qy, qz])              # qTOm([qw, qx, qy, qz])
...
def overlapped_boxes_3d_iou(single_box, box_list):
    overlap = np.zeros(len(box_list))  
    box_vertices1 = Box(vertices=single_box)
    box1 = Box.from_transformation(np.array(box_vertices1.rotation), 
        np.array(box_vertices1.translation), np.array([1., 1., 1.]))
    iou = IoU(box1, box1)
    for i in range(len(box_list)):
        box_vertices2 = Box(vertices=box_list[i])
        box2 = Box.from_transformation(np.array(box_vertices2.rotation), 
        np.array(box_vertices2.translation), np.array([1., 1., 1.]))
        iou.setBox(box1, box2)  
        overlap[i] = iou.iou()
    return overlap
