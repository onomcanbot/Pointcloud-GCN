...
def quaternion_from_euler(roll, yaw, pitch):
    """
...
"""
    ...
    def get_ply_label(self, frame_idx, no_orientation=False):

        self._label_dir="../dataset/label/converted/"
        ...
        with open(label_file, 'r') as f:
            for line in f:
                ...
                label['name'] = fields[0]
                # 0=visible 1=partly occluded, 2=fully occluded, 3=unknown
                label['x3d'] =  float(fields[1])
                label['y3d'] =  float(fields[2])
                label['z3d'] =  float(fields[3])
                label['height'] =  float(fields[4])
                label['width'] =  float(fields[5])
                label['length'] =  float(fields[6])
                label['pitch'] =  float(fields[7])  # pitch
                label['yaw'] =  float(fields[8])    # yaw
                label['roll'] =  float(fields[9])   # roll
                label_list.append(label)
        return label_list
    ...
    def boxes_3d_to_line_set(self, boxes_3d, boxes_color=None):
        ...
        colors = []
        for i, box_3d in enumerate(boxes_3d):
            x3d, y3d, z3d, l, h, w, pitch, yaw, roll = box_3d

            pitch_R = np.array([[1,        0,          0  ],         # pitch
                      [0, np.cos(pitch), -np.sin(pitch)],
                      [0, np.sin(pitch),  np.cos(pitch)]]);
                                                                         
            yaw_R = np.array([[np.cos(yaw),  0,  np.sin(yaw)],        # yaw
                      [0,            1,  0          ],
                      [-np.sin(yaw), 0,  np.cos(yaw)]]);

            roll_R = np.array([[np.cos(roll), -np.sin(roll),  0],     # roll
                      [np.sin(roll),  np.cos(roll),  0],
                      [0,              0,              1]]);

            R = roll_R @ yaw_R @ pitch_R                              # R
            ...
            corners = np.array([[  0 ,   0 ,   0 ],
                                [-l/2, -h/2, -w/2],  # back down left
                                [-l/2, -h/2,  w/2],  # back down right
                                [-l/2,  h/2, -w/2],  # back up left
                                [-l/2,  h/2,  w/2],  # back up right
                                [ l/2, -h/2, -w/2],  # front down left
                                [ l/2, -h/2,  w/2],  # front down right
                                [ l/2,  h/2, -w/2],  # front up left
                                [ l/2,  h/2,  w/2]]) # front up right
            ...
            edges.append(
               np.array([[1, 5], [2, 6], [3, 7], [4, 8],           # lines along x-axis
                         [1, 3], [5, 7], [2, 4], [6, 8],           # lines along y-axis
                         [1, 2], [3, 4], [5, 6], [7, 8]]) +i*9)    # lines along z-axis
    ...
    def boxes_3d_to_line_set_qt(self, boxes_3d, boxes_color=None):
        # for quaternion
        points = []
        edges = []
        colors = []
        for i, box_3d in enumerate(boxes_3d):
            x3d, y3d, z3d, l, h, w, qw, qx, qy, qz = box_3d

            qm = self.qTOm([qw, qx, qy, qz])
            ...
            r_corners = corners.dot(np.transpose(qm))
            cam_points_xyz = r_corners+np.array([x3d, y3d, z3d])
            points.append(cam_points_xyz)
        ...
        for label in labels:
            ...
            if obj_cls >= 1 and obj_cls <= 8:
                ...
                if roll < 0.25*np.pi:
                    ...
                    boxes_3d[mask, 0, :] = (label['x3d'], label['y3d'],
                        label['z3d'], label['length'], label['height'],
                        label['width'], pitch, yaw, roll)                    # pitch, yaw, roll
                    valid_boxes[mask, 0, :] = 1
                    #print("hor")
                else:
                    # vertical
    ...
    def quaternion_from_euler(self, roll, yaw, pitch):
        """
        Converts euler roll, pitch, yaw to quaternion (w in last place)
        quat = [x, y, z, w]
        """
    ...
    def qTOm(self, q):
        ret = np.zeros((3,3))
        ret[0][0] = 1 - 2 * (q[1] * q[1] + q[2] * q[2])
        # inverse
        ret[0][1] = -2 * (q[0] * q[1] - q[3] * q[2])
        ret[0][2] = 2 * (q[0] * q[2] + q[3] * q[1])

        ret[1][0] = 2 * (q[0] * q[1] + q[3] * q[2])
        # inverse
        ret[1][1] = -1 + 2 * (q[0] * q[0] + q[2] * q[2])
        ret[1][2] = 2 * (q[1] * q[2] - q[3] * q[0])

        # inverse
        ret[2][0] = -2 * (q[0] * q[2] - q[3] * q[1])
        ret[2][1] = 2 * (q[1] * q[2] + q[3] * q[0])
        # inverse
        ret[2][2] = -1 + 2 * (q[1] * q[1] + q[0] * q[0])
        return ret
